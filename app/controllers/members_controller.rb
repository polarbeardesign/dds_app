class MembersController < ApplicationController
  ...
  def index
    ...
    @new_members = Member.find_by_sql("SELECT members.id, members.caf_col_no, members.caf_join_date, members.created_at, last_name, first_name, users.email FROM members LEFT JOIN users on users.id = user_id LEFT JOIN assignments ON assignments.user_id = members.user_id WHERE assignments.role_id IS NULL AND members.id > 144;")
